...
def process_pdf_with_ocrmypdf(input_path, output_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True
        elif "PriorOcrFoundError" in result.stderr:
            # 如果文件已经包含OCR文本，使用--skip-text参数重新处理
            print(f"文件已包含文本，使用--skip-text参数重新处理: {os.path.basename(input_path)}")
            cmd_skip_text = [
                'ocrmypdf',
                '-l', 'chi_sim',  # 简体中文
                '--optimize', '3',  # 最高级别压缩
            ...
            print(f"重新处理命令: {' '.join(cmd_skip_text)}")
            result_skip = subprocess.run(cmd_skip_text, capture_output=True, text=True, timeout=600)
            
            if result_skip.returncode == 0:
                print(f"成功处理: {os.path.basename(input_path)}")
                return True
            else:
                print(f"重新处理失败: {os.path.basename(input_path)}")
                print(f"错误信息: {result_skip.stderr}")
                return False
        else:
