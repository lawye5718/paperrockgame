            ...
            if "lines" in block:
                for line in block["lines"]:
                    for span in line["spans"]:
                        text_detailed += span["text"]
                    text_detailed += "\n"
        
        # 使用更有效的文本
        text_content = text_detailed if len(text_detailed) > len(text_simple) else text_simple
...
def is_toc_page(text):
    ...
    for line in lines[:3]:  # 检查前3行
        ...
        if clean_line and "目录" in clean_line:
            # 确保"目录"在行的前半部分，避免误判
            if clean_line.find("目录") < len(clean_line) / 2:
                return True
    return False
...
def process_pdf_file(pdf_path, output_base_dir):
    ...
    try:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            ...
            if cover_text:
                # 只保留前2000个字符
                preview_text = cover_text[:2000]
                if len(cover_text) > 2000:
                    preview_text += "\n... (内容截断)"
                f.write(f"```\n{preview_text}\n```\n")
            else:
                f.write("无文本内容\n")
